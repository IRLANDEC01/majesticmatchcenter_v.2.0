...
## 🎯 Текущая Задача: Рефакторинг "Шаблонов Карт"

> **Статус:** Ожидание.
> **Цель:** Восстановить утраченный функционал (архивация, фильтрация), исправить проблемы с кэшированием и решить баг с потерей фокуса в UI, применяя лучшие практики для повышения надежности и производительности.

### **Фаза 1: Восстановление Фундамента (Модель и API)**

-   **[ ] Шаг 1: Обновление модели `MapTemplate`**
    -   **Задача:** Вернуть в схему Mongoose поля для "мягкого удаления".
    -   **Действия:**
        1.  Открыть `src/models/map/MapTemplate.js`, добавить поле `archivedAt` (тип `Date`) и виртуальное поле `isArchived`.
        2.  **`💡 Улучшение:`** Добавить **индекс** на поле `archivedAt` (`{ archivedAt: 1 }`). Это значительно ускорит будущие запросы на фильтрацию по архивным записям.

-   **[ ] Шаг 2: Восстановление API для архивации (`.../archive`).**
    -   **Задача:** Вернуть маршрут для "мягкого удаления" шаблона.
    -   **Действия:** Восстановить `PATCH` маршрут, который устанавливает дату в `archivedAt`.

-   **[ ] Шаг 3: Восстановление API для разархивации (`.../restore`).**
    -   **Задача:** Вернуть маршрут для восстановления шаблона из архива.
    -   **Действия:** Создать `PATCH` маршрут, который сбрасывает `archivedAt` в `null`.

-   **[ ] Шаг 4: Реализация фильтрации на бэкенде.**
    -   **Задача:** Научить `GET /api/admin/map-templates` фильтровать шаблоны по статусу.
    -   **Действия:**
        1.  Модифицировать `route.js` для обработки query-параметра `status`.
        2.  **`💡 Улучшение:`** Реализовать **нормализацию и валидацию** входящего параметра `status` (например, через Zod-схему), чтобы защититься от некорректных значений.

### **Фаза 2: Интеграция Улучшений (Кэширование)**

-   **[ ] Шаг 5: Внедрение `revalidatePath`.**
    -   **Задача:** Добавить правильную инвалидацию кэша Next.js.
    -   **Действия:**
        1.  Добавить `revalidatePath('/admin/map-templates')` во все изменяющие маршруты.
        2.  **`💡 Улучшение:`** Убедиться, что вызов `revalidatePath` происходит **до** `NextResponse.json()` для корректной работы.

-   **[ ] Шаг 6: Проверка и улучшение внутреннего кэша (Redis).**
    -   **Задача:** Обеспечить консистентность и точность нашего Redis-кэша.
    -   **Действия:**
        1.  Проверить, что в `map-template-repo.js` все изменяющие методы вызывают инвалидацию.
        2.  **`💡 Улучшение:`** Перейти на **ключи кэша, зависящие от запроса** (например, `map-templates:status=archived:page=1`). Это позволит делать более точную и безопасную инвалидацию.

### **Фаза 3: Стабилизация и Тестирование**

-   **[ ] Шаг 7: Адаптация и расширение тестов.**
    -   **Задача:** Покрыть тестами всю новую и восстановленную логику.
    -   **Действия:**
        1.  Написать **интеграционные тесты** для API архивации/восстановления.
        2.  **`💡 Улучшение:`** Добавить **модульные (unit) тесты** для `map-template-repo.js`, чтобы изолированно проверить логику `archive()`, `restore()` и `findByStatus()`.
        3.  Добавить моки для `revalidatePath` в интеграционных тестах.

-   **[ ] Шаг 8: Финальная проверка.**
    -   **Задача:** Убедиться в стабильности всего проекта.
    -   **Действия:** Запустить полный набор тестов.
        *   **`💡 Улучшение:`** Будем использовать `jest --runInBand` только для финальной проверки на наличие "зависших" операций, а для обычной разработки — стандартный запуск, чтобы не замедлять процесс.

### **Фаза 4: Исправление UI (после завершения бэкенда)**

-   **[ ] Шаг 9: Решение проблемы с потерей фокуса.**
    -   **Задача:** Исправить потерю фокуса в поле поиска на странице `/admin/map-templates`.
    -   **Действия:**
        1.  Реализовать паттерн с разделением локального и глобального состояния, как описано в `architecture/swr_focus_issue_solution.md`.
        2.  **`💡 Улучшение:`** Обернуть обновление состояния, которое запускает поиск, в **`useTransition`** из React. Это сделает интерфейс неблокирующим и более отзывчивым.
