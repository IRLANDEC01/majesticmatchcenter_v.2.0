...
## 3. Ключевые принципы надежности (Обновлено в Августе 2024)

В ходе рефакторинга и устранения ошибки `Module not found: Can't resolve 'bull'` были выработаны и закреплены следующие архитектурные принципы, обязательные к соблюдению.

### 3.1. Гарантированная изоляция серверного кода
*   **Проблема:** Случайный импорт серверного модуля (например, работающего с БД или очередями) в клиентский компонент приводит к ошибке сборки и потенциальной утечке данных.
*   **Решение:** Пакет `server-only`. Мы добавляем `import 'server-only';` в начало каждого файла, который **ни при каких условиях** не должен попасть в клиентский бандл.
*   **Где применяется:**
    *   `src/lib/db.js`, `src/lib/redis.js`
    *   Все файлы в `src/models/`
    *   Все файлы в `src/lib/repos/` и `src/lib/domain/`
    *   Все файлы, работающие с очередями (`src/queues/`).
*   **Результат:** Любая попытка импортировать такой модуль на клиенте немедленно вызовет ошибку сборки, предотвращая проблему на самом раннем этапе.

### 3.2. Централизованная политика кэширования
*   **Проблема:** "Магические числа" (случайные TTL) и строки (имена тегов), разбросанные по коду, усложняют поддержку и отладку кэша.
*   **Решение:** Создан единый файл `src/lib/cache/cache-policy.ts`.
*   **Что он содержит:**
    *   `cacheTTL`: Объект, где ключами являются префиксы кэша (например, `'players:list'`), а значениями — время жизни в секундах.
    *   `cacheTags`: Функции-хелперы для генерации консистентных имен тегов (например, `cacheTags.player('some-id')`).
*   **Результат:** Управление политикой кэширования в одном месте. Легко найти, изменить и понять, как долго живет тот или иной кэш.

### 3.3. Правильное распределение ответственности (Сервис > Репозиторий)
*   **Проблема:** Изначально `BaseRepo` отвечал не только за работу с БД, но и за вызов очереди для обновления поискового индекса. Это нарушало принцип единственной ответственности.
*   **Решение:** Логика вызова очереди (`searchQueue.add()`) была полностью удалена из слоя репозиториев и перенесена в **сервисный слой** (`-service.ts`).
*   **Почему это правильно:**
    *   **Репозиторий:** Его единственная задача — быть адаптером к базе данных (CRUD-операции). Он ничего не знает о бизнес-процессах.
    *   **Сервис:** Оркестрирует бизнес-логику. Именно он знает, что после успешного сохранения данных в БД (через репозиторий) нужно запустить дополнительный процесс (обновить поисковый индекс).
*   **Результат:** Более чистая, предсказуемая и тестируемая архитектура, соответствующая принципу слоистости.

---

## 4. Практическая реализация и стратегии
