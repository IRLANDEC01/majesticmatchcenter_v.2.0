...
#### Создание карты
1. Администратор создает карту в рамках  турнира
2. Выбирает шаблон карты (опционально)
3. Добавляет семьи/команды-участники из участников турнира
4. Добавляет игроков для каждой семьи/команды
5. Устанавливает дату и время проведения
6. Публикует карту (статус автоматически устанавливается как planned)

#### Автоматическая активация карты
1. Система автоматически меняет статус на active, когда наступает установленное время начала карты
2. Система проверяет наличие участников
3. Карта становится доступна для загрузки статистики
4. Администратор может вручную поменять время, статус соответственно поменяется атоматически если нужно

#### Изменение расписания карты
1. Администратор может изменить дату и время начала карты
2. Система автоматически корректирует статус в зависимости от нового времени:
   - Если новое время в будущем и карта активна - статус становмтся planned
   - Если новое время в будущем и карта planned - статус остается planned
   - Если новое время в прошлом и карта planned - статус меняется на active

#### Завершение карты
1. Администратор загружает статистику json файл
2. Система валидирует статистику (наличие kills, deaths, damage)
3. Администратор определяет победителя (семья или команда)
4. Система начисляет рейтинг игрокам (1 kill = 1 рейтинг)
5. Администратор устанавливает изменения рейтинга для семей
6. Администратор завершает карту
7. Система обновляет статистику игроков и семей:
   - Обновляется общая статистика игроков (kills, deaths, damage)
   - Обновляется статистика по оружию (kills, damage, accuracy)
   - Обновляется месячная и годовая статистика игроков
   - Обновляется статистика участия в семьях
   - Обновляется статистика семей (wins, maps played)
   - Создаются записи о достижениях (MVP, most kills, best K/D)
   - Обновляются рейтинговые таблицы (топы игроков и семей)
...
- Карта не может быть завершена без загруженной статистики, выбранного MVP карты, выбранного победителя и установленного рейтинга для хотя бы одной семьи
