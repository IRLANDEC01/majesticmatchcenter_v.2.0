...
### 1.4 Паттерн хранения истории участий и рейтингов

Для управления историей участий и изменениями рейтинга в проекте используется **единый и симметричный подход** для игроков и семей, основанный на принципе "одно участие — одна запись". Это обеспечивает целостность данных и высокую производительность запросов.

**Компоненты паттерна:**

1.  **Модель Участия в Карте (`PlayerMapParticipation`, `FamilyMapParticipation`)**
    *   **Роль:** Основной источник правды для **контекста** одного матча.
    *   **Реализация:** Отдельные коллекции `player_map_participations` и `family_map_participations`.
    *   **Содержит:** Полную информацию об участии в одной карте: игровую статистику (`kills`, `deaths`), результат (`isWinner`) и полный контекст изменения рейтинга (`previousRating`, `newRating`, `ratingChange`).
    *   **Использование:** Применяется для построения детальной истории игр на публичных страницах игроков и семей.

2.  **Основная Модель (`Player`, `Family`)**
    *   **Роль:** Хранит только **текущий, актуальный рейтинг**.
    *   **Реализация:** Поле `rating` в схемах `Player` и `Family`. Это денормализованное значение для мгновенного доступа.
    *   **Использование:** Отображение текущего рейтинга в профилях, использование в лидербордах.

3.  **Статистическая Модель (`PlayerStats`, `FamilyStats`)**
    *   **Роль:** Денормализованный **кэш** для быстрого построения сложных лидербордов.
    *   **Реализация:** Поля `monthly`, `yearly`, `overall` в схемах `PlayerStats` и `FamilyStats`.
    *   **Использование:** Агрегирует данные из записей участий для быстрого ответа на запросы "топ-10 за месяц/год" без необходимости сканировать всю историю.

**Правило применения:**

*   **При завершении карты:**
    1.  Сервисы (`rating-service`, `statistics-service`) создают/обновляют по одной записи в `PlayerMapParticipation` для каждого игрока и `FamilyMapParticipation` для каждой семьи.
    2.  Эти записи содержат всю информацию о матче.
    3.  Одновременно с этим атомарно обновляется поле `rating` в основных моделях `Player` и `Family`.
    4.  Также обновляются агрегированные данные в `PlayerStats` и `FamilyStats`.

Этот подход обеспечивает как высокую производительность на чтение для частых сценариев, так и полноту и целостность исторических данных.
