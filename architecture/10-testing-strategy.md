...
10. **❌ Агрегация данных "на лету" вместо создания целостных записей.**
    *   **Проблема:** Попытка вывести историю участий одной сущности (например, Семьи) путем сканирования и агрегации данных из коллекции участий другой сущности (например, `PlayerMapParticipation`).
    *   **Почему это плохо:** Это приводит к двум главным проблемам. **Во-первых, к катастрофической неэффективности**: запрос для одной сущности требует полного сканирования (full collection scan) другой, огромной коллекции, что не масштабируется. **Во-вторых, к потере контекста**: в дочерней записи (участие игрока) может не быть всей нужной информации о родительской (например, был ли результат победой для всей семьи).
    *   **Решение:** Для каждой сущности, чью историю участий необходимо отслеживать, должна существовать **собственная, целостная модель участия** (например, `FamilyMapParticipation`). Это обеспечивает быстрый индексный поиск и гарантирует, что вся контекстная информация о событии хранится в одной, логически связанной записи. 